...
#include "global.hpp"
  ...
  static auto httpRespDirHtml(httpd_req_t *req, const char *dirpath) -> esp_err_t {
    ...
    if (dir == nullptr) {
      LOG_E("Failed to stat dir : {} ({})", dirpath, strerror(errno));
      httpd_resp_send_err(req, HTTPD_404_NOT_FOUND, "Directory does not exist");
      return ESP_FAIL;
    }

    httpd_resp_sendstr_chunk(
      req, "<!DOCTYPE html><html>"
      "<head>"
      "<meta charset=\"UTF-8\">"
      ...
      "table.list th {border: 1px solid #077C95; padding: 12px 8px; background-color: "
      "#077C95; color: white;}"
    ...
    httpd_resp_sendstr_chunk(
      req, "<table class=\"fixed list\" id=\"sorted\" next_sort=\"asc\">"
      "<colgroup><col width=\"70%\"/><col width=\"8%\"/><col width=\"14%\"/><col "
      "width=\"8%\"/></colgroup>"
      "<thead><tr><th onclick=\"sortTable(0)\">Name</th><th>Type</th><th>Size "
    ...
    while ((entry = readdir(dir)) != NULL) {
      ...
      if (stat(entrypath.c_str(), &entry_stat) == -1) {
        LOG_E("Failed to stat {} : {}", entrytype, entry->d_name);
        continue;
      }

      itoa(entry_stat.st_size, entrysize, 10);
      LOG_I("Found {} : {} ({} bytes)", entrytype, entry->d_name, entry_stat.st_size);
    ...
    httpd_resp_sendstr_chunk(
      req, "<script>window.addEventListener(\"load\", function(){sortTable(0);})</script>"
      "</body></html>");
  ...
  -> std::string {
    LOG_D("getPathFromUri, base_path: {}, uri: {}", base_path, uri);
  ...
  static auto downloadHandler(httpd_req_t *req) -> esp_err_t {
    LOG_D("downloadHandler({}) {}", req->uri, (int)req->user_ctx);
    ...
    LOG_D("filepath = {}, filename = {}", filepath, filename);
    ...
    if (stat(filepath.c_str(), &fileStat) == -1) {
      ...
      LOG_E("Failed to stat file : {}", filepath);
    ...
    if (!fd.is_open()) {
      LOG_E("Failed to read existing file : {}", filepath);
      httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Failed to read existing file");
      return ESP_FAIL;
    }

    LOG_I("Sending file : {} ({} bytes)...", filename, fileStat.st_size);
  ...
  static auto uploadHandler(httpd_req_t *req) -> esp_err_t {
    ...
    if (filename[filename.size() - 1] == '/') {
      LOG_E("Invalid filename : {}", filename);
      httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Invalid filename");
      return ESP_FAIL;
    }

    if (stat(filepath.c_str(), &fileStat) == 0) {
      LOG_E("File already exists : {}", filepath);
      httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "File already exists");
      return ESP_FAIL;
    }

    if (req->content_len > MAX_FILE_SIZE) {
      LOG_E("File too large : {} bytes", req->content_len);
    ...
    fd.open(filepath.c_str(), std::ios::binary);
    if (!fd.is_open()) {
      LOG_E("Failed to create file : {}", filepath);
      httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Failed to create file");
      return ESP_FAIL;
    }

    LOG_I("Receiving file : {}...", filename);
    ...
    while (remaining > 0) {
      ...
      LOG_I("Remaining size : {}", remaining);
  ...
  static auto httpServerStart() -> esp_err_t {
    ...
    LOG_I("Starting HTTP Server on port {}...", port);
    esp_err_t res = httpd_start(&server, &httpd_config);
    if (res != ESP_OK) {
      LOG_E("Failed to start file server ({})!", esp_err_to_name(res));
      return ESP_FAIL;
    }
  ...
  auto startWebServer(WebServerMode serverMode) -> bool {
    ...
    if (((serverMode == WebServerMode::STA) && wifi.startSta()) ||
      ...
      if (httpServerStart() == ESP_OK) {
        ...
        MsgViewer::show(
          MsgViewer::MsgType::ALERT, true, true, "Web Server Failed",
          "The Web server was not able to start. Correct the situation and try again.");
        wifi.stop();
      }
    } else {
      MsgViewer::show(
        MsgViewer::MsgType::ALERT, true, true, "WiFi Startup Failed",
  ...
  auto startWebServerHeadless(WebServerMode serverMode) -> bool {
    ...
    esp_ip4_addr_t ip = wifi.getIpAddress();
    LOG_I("S7: Web server running at {}.{}.{}.{}", IP2STR(&ip));
    return true;
  }
